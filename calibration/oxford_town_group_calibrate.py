...
def projection_matrices_oxford_town_group():
...
def convert_background(multiplier=10):
    img = cv2.imread('oxford_town_group_background.png')
    HEIGHT = img.shape[0]
    WIDTH = img.shape[1]
    DEPTH = img.shape[2]
    ...
    w_cal = 50 * multiplier
    img_cal = np.zeros((h_cal, w_cal, 3))
    in_mat, ex_mat = projection_matrices_oxford_town_group()
    for i in range(h_cal):
        for j in range(w_cal):
    ...
    cv2.imwrite('oxford_town_group_background_calibrated.png', img_cal)
...
def save_transforamtion_matrix():
    ...
    np.savetxt('oxford_town_group_matrix_cam2world.txt', M_c2w)
